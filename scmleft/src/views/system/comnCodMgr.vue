  ...
  <div id="comnCodMgr">
        ...
        <tbody>
          ...
          <template v-else>
            <tr v-for="item in items" :key="item.grp_cod">
              ...
              <td
                id="groupTitle"
                @click="searchdetail(item.grp_cod, item.grp_cod_nm)"
              >
                {{ item.grp_cod }}
              </td>
    ...
    <div>
      <p class="conTitle">
        <span>그룹 코드 상세</span>
        <span class="fr">
          <a class="btn btn-primary mx-2" @click="detailPop()" name="modal">
            <span>신규등록</span>
          </a>
        </span>
      </p>
          ...
          <tbody>
            ...
            <template v-else>
              <tr
                ...
                :key="item.grp_cod"
                @click="detailPop(item.dtl_cod, item.dtl_cod_nm, item.use_poa)"
              >
                <td>{{ item.row_num }}</td>
                <td>{{ item.dtl_cod }}</td>
...
export default {
  ...
  methods: {
    // uwfa
    searchdetail: function (group_code, group_name) {
      let vm = this;
      console.log(group_code + '  ::  ' + group_name);
      this.bkgrp_cod = group_code;
      this.bkgrp_cod_nm = group_name;
    ...
    clickCallback2: function (pageNum) {
      ...
      this.searchdetail(this.bkgrp_cod, this.bkgrp_cod_nm);
    },
    page: function () {
    ...
    detailPop: async function (detailCode, detailName, use) {
      if (detailCode == null || detailCode == '') {
        // 상세코드 등록
        this.action2 = 'I';
        const modal = await openModal(detailModal, {
          title: '상세코드 등록',
          grpcd: this.bkgrp_cod,
          grpnm: this.bkgrp_cod_nm,
          detailCode: '',
          action: this.action2,
        });
        ...
        console.log(use);
        // 상세코드 수정
        console.log('name check :: ' + detailName);
        this.action2 = 'U';
        const modal = await openModal(detailModal, {
          title: '상세코드 수정',
          grpcd: this.bkgrp_cod,
